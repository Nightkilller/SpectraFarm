...
class GroundTruthDatasetStatus(str, Enum):
    ...
    INVALID = "INVALID"
    REQUIRES_REVIEW = "REQUIRES_REVIEW"
    SEHORE_GROUND_TRUTH = "SEHORE_GROUND_TRUTH"


class DatasetProvenance(BaseModel):
    """Metadata tracking authoritative provenance, licensing, and geographic limits."""
    dataset_name: str
    dataset_version: str = "1.0"
    source_url: Optional[str] = None
    license: str = "CC-BY-4.0"
    geographic_region: str
    is_sehore_ground_truth: bool = False
    label_collection_method: Optional[str] = None
...
class GroundTruthRecord(BaseModel):
    ...
    latitude: float = Field(..., ge=-90.0, le=90.0, description="Field centroid latitude WGS84")
    longitude: float = Field(..., ge=-180.0, le=180.0, description="Field centroid longitude WGS84")
    crop_type: str = Field(..., description="Observed crop type (e.g. Wheat, Soybean, Mustard, Gram, Other)")
    season: str = Field(..., description="Agricultural season (e.g. Rabi, Kharif, Zaid)")
    reference_date: date = Field(..., description="Actual survey or ground validation date")
    source: str = Field(..., description="Authoritative source (e.g. Field Survey KVK, State Extension Registry)")
...
class GroundTruthValidationReport(BaseModel):
    """Audit report generated by the ground truth validator."""
    dataset_name: str
...
class GroundTruthDataset(BaseModel):
    """Container for ground truth dataset with audit summary."""
    dataset_name: str
    provenance: Optional[DatasetProvenance] = None
    records: list[GroundTruthRecord] = Field(default_factory=list)
    validation_report: Optional[GroundTruthValidationReport] = None
    data_source: DataSource = DataSource.LIVE
